...
export class AngularFileUploaderComponent implements OnChanges {
  ...
  constructor(@SkipSelf() private http: HttpClient,private messageService: MessageService,private confirmationService: ConfirmationService) {}

  ngOnInit(): void {
    this.http.request((this.config.downloadAPI.method||'GET').toUpperCase(),this.config.downloadAPI.url,{
      reportProgress: true,
      observe: 'events',
      headers: this.config.downloadAPI.headers,
      params: this.config.downloadAPI.params,
      responseType: this.config.downloadAPI.responseType,
      withCredentials: this.config.downloadAPI.withCredentials,
    }).subscribe(event => {
      event.body?.forEach((file: any) => {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if(changes['config']&&this.config) {
      ...
      this.downloadAPI=this.config.downloadAPI;
  ...
  onChange(event: any) {
    ...
    for(let i=0;i<fileList.length;i++) {
      const currentFileExt: any=fileExtRegExp
        .exec(fileList[i].name)??[1].toString()
        ...
        (event) => {
          ...
          if(event.type===HttpEventType.UploadProgress) {
            ...
            if(event.status===200||event.status===201) {
              ...
              if(!isError) {
                this.messageService.add({key: 'tc',severity: 'success',summary: 'Success',detail: this.replaceTexts?.afterUploadMsg_success});
  ...
  removeFileLogic(i: any,sf_na: any) {
    if(sf_na==='sf') {
  ...
  downloadFile(i: any,sf_na: any) {
    if(sf_na==='sf') {
      /* let url = window.URL.createObjectURL(this.allowedFiles[i]);
        let a = document.createElement('a');
        document.body.appendChild(a);
        a.setAttribute('style', 'display: none');
        a.href = url;
        a.download = this.allowedFiles[i].name;
        a.click();
        window.URL.revokeObjectURL(url);
        a.remove(); */
      saveAs(this.allowedFiles[i],this.allowedFiles[i].name);
    } else if(sf_na==='na') {
